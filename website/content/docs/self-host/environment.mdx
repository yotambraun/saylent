...
| `INNGEST_SIGNING_KEY` | cloud, own-server (production) | Required so `/api/inngest` is authenticated; without it in production, paid runs are externally triggerable |
...
| `APP_SECRET` | unset | The pgcrypto passphrase that encrypts provider API keys saved from `/admin/providers`, so an operator can change a key or a per-role model with no redeploy. Generate with `openssl rand -hex 32` (min 16 chars). Unset = keys are environment-only and the console says so; models stay editable either way. Never stored in the database - losing or changing it makes every console-saved key unreadable |
| `RESEND_API_KEY` + `EMAIL_FROM` | unset | Transactional email (welcome, report-ready, movement, verify reminders) - inert without both |
...
| `AUDIT_PROFILE` | full | The app runs the full profile (23 questions, about $2.50 to $4.00 per run) and shows the estimate before every run. `smoke` (6 questions) is a development profile and is refused for real users in production |
...
## The docs site's own instant check
