---
title: The site check
description: What AI crawlers can read on your site. Free, no keys.
---

```bash
npx saylent gate-check example.com   # free, no keys, nothing stored
```

That is the site check: which AI crawlers your site lets in, and which ones it
turns away without telling you. It reads your robots.txt agent by agent,
fetches a page as each crawler to see what really comes back, and reads the
JSON-LD and meta directives on the pages it crawled. No sign-up, no keys, no
LLM calls, nothing stored.

The box on this site's [home page](/) runs the same code, so the verdict you
see there is the verdict you get in your own terminal. Running it yourself has
no rate limit, reads more of your site, and sends your domain to nobody.
...
variables](/docs/self-host/environment)). With the variable unset, on a fork, a
